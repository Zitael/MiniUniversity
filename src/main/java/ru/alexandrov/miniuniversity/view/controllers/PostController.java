...
import ru.alexandrov.miniuniversity.dao.converter.GroupToDtoConverter;
import ru.alexandrov.miniuniversity.dao.converter.TeacherToDtoConverter;
...
public class PostController {
    ...
    private final GroupToDtoConverter groupToDtoConverter;
    ...
    ResponseEntity<Set<GroupDTO>> getGroupsByTeacher(@RequestBody TeacherNameRequest request) {
        Teacher t = teacherRepository.findByName(request.getTeacherName());
        if (t == null) {
            log.error("Teacher has not found with name {}", request.getTeacherName());
            return notFound().build();
        }
        TeacherDTO teacherDTO = teacherToDtoConverter.convert(t);
    ...
    ResponseEntity<Set<StudentDTO>> getStudentsByGroup(@RequestBody GroupNameRequest request) {
        ...
        GroupDTO groupDTO = groupToDtoConverter.convert(group);
